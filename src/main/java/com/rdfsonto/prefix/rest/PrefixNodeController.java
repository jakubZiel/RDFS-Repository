...
public class PrefixNodeController
{
    ...
    @GetMapping
    ResponseEntity<?> getPrefixByProjectId(final long projectId)
    {
        return ResponseEntity.ok(prefixNodeService.findAll(projectId));
    }

    @PostMapping
    ResponseEntity<?> updatePrefix(@RequestBody final Map<String, String> prefixes, final long projectId)
    {
