...
{
    @Query("MATCH (n:Resource)-[relation]->(neighbour:Resource) WHERE id(n) = $nodeId RETURN neighbour, type(relation) as relation")
    ...
    @Query("MATCH (n:Resource)<-[rel]-(neighbour:Resource) WHERE id(n) IN $nodeIds RETURN neighbour, type(rel) as relation, id(n) as source")
